/**
 * Gamba affiliate standings, read from the krush.gg affiliate API.
 *
 * Documented shape:
...
export type GambaEntry = { name: string; wagered: number };
 ...
 * entrants to fill it.
 */
export async function fetchGambaLeaderboard(
  startMs: number,
  endMs: number,
  limit: number,
): Promise<GambaEntry[] | null> {
  const key = process.env.GAMBA_API_KEY?.trim();
  if (!key) {
    console.error("GAMBA_API_KEY is not set — leaderboard cannot be fetched");
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Gamba leaderboard HTTP ${response.status}`);
    ...
    if (payload.code && payload.code !== 200) {
      throw new Error(`Gamba leaderboard code ${payload.code}: ${payload.msg ?? ""}`);
    }
    ...
    console.error("Gamba leaderboard fetch failed:", error);
    return null;
  }
}
